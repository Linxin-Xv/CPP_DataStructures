...
### 1.前序遍历（根左右），遍历过程
#### (1)先将根节点入栈；
#### (2)如果栈不为空则循环下面的步骤，直至栈为空；
##### [1]栈顶元素出栈并访问（显示节点值）这个元素；
##### [2]如果这个被访问的元素右子节点不为空，则把其右子节点入栈；
##### [3]如果这个被访问的元素左子节点不为空，则把其左子节点入栈；
``` cpp
template <typename T>
void BinaryTree<T>::prePrder_noRecu(BinaryTreeNode<T>* tNode)
{
	if (tNode == nullptr)
	{
		return;
...
```
### 2.中序遍历（左根右），遍历过程
#### (1)先将根节点入栈，并把根节点标记为当前节点；
#### (2)如果栈不为空则循环下面的步骤，直至栈为空；
##### [1]如果当前节点的左子节点不为空，则循环做两件事，直到当前节点的左子节点为空；
##### i.将当前节点的左子节点入栈
##### ii.将当前节点的左子节点重新标记为当前节点；
##### [2]栈顶元素出栈并访问；
##### [3]如果被访问节点的右子节点不为空，则把自己的右节点设定为当前节点，并且入栈；
``` cpp
//非递归方式实现中序遍历
template <typename T>
void BinaryTree<T>::inPrder_noRecu()
{
	inPrder_noRecu(root);
}
template <typename T>
void BinaryTree<T>::inPrder_noRecu(BinaryTreeNode<T>* tNode)
{
	if (tNode == nullptr)
	{
		return;
...
struct BTNode_extra
{
	...
	int pointSign;//1代表左孩子，其余代表右孩子
};
```
#### (2)把根节点标记为当前节点;
#### (3)循环下面的步骤(循环1);
##### [1]如果当前节点不为空，则循环（循环2）做两件事，知道当前节点为空
##### i.将当前节点同左子树标记一起入栈，意味着后续会继续将当前节点的左子节点压栈；
##### ii.将当前节点的左子节点重新标记为当前节点；
##### [2]如果栈不为空，则循环（循环3）做如下事情；
##### i.栈顶元素出栈
##### ii.如果出栈的元素有左子树标记，则把该元素的左子树标记改为右子树标记；
##### a.重新将该元素入栈；
##### b.将该元素的右子节点标记为当前节点；
##### c.终止循环3
##### iii.如果出栈的元素有右子树标记，则进行访问；
#### [3]如果栈为空则遍历结束（循环1结束）
``` cpp
//非递归方式实现后序遍历
template <typename T>
...
```
## 13.结合栈和队列实现递归和非递归前、中、后序遍历、层序遍历以及完成相关操作的完整代码
``` cpp
#include <iostream>
using namespace std;
/*栈实现区域*/
